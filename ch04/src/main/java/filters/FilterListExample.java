...
public class FilterListExample {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (Table table = HBaseUtils.getTable(HBaseConstants.TEST_TABLE)) {
            ...
            Filter filter1 = new RowFilter(CompareOperator.GREATER_OR_EQUAL, new BinaryComparator(Bytes.toBytes("row-03")));
            filters.add(filter1);

            Filter filter2 = new RowFilter(CompareOperator.LESS_OR_EQUAL, new BinaryComparator(Bytes.toBytes("row-06")));
            filters.add(filter2);

            Filter filter3 = new QualifierFilter(CompareOperator.EQUAL, new RegexStringComparator("col-0[03]"));
            filters.add(filter3);
